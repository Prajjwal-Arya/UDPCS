...
with open("../privateBC.pem") as ff:
    sk = SigningKey.from_pem(ff.read())
f = open('../rsaABkey.pem', 'rb')  
f1 = open('../rsaBCpub.pem', 'rb')  
vk = VerifyingKey.from_pem(open("../publicAB.pem").read())
...
class Client():
    ...
    def send_msg(self, sock):
        ...
        while True:
            print val 
            print '------------------------val-----------------------------'
            print 'Enter 1 to decrypt the message : '
            choose =input()
            if choose == 1:
                file1 = open('log.txt','r')
                val = file1.read()
                result = re.split('1111111',val)
                msg = result[0]
                sigmsg = result[1]
                try:
                    vk.verify(sigmsg, msg)
                ...
                val = pubKey.encrypt(val,32)
                val = convertTuple(val)
                sig = sk.sign(val)
                val = val + '1111111' + sig 
                print '-----------------------sig------------------------------'
                print val
                print '-----------------------sig------------------------------'
                sock.sendto(val, self.target)
                print '-----------------------------------------------------'
