...
f1 = open('../rsaABpub.pem', 'rb')  
with open("../privateAB.pem") as ff:
...
class Client():
    ...
    def send_msg(self, sock):
        ...
        while True:
            ...
            bb += pay 
            if balance <= bb:
                print 'You cant send anymore ethers.'
            else:
                print '----------------------balance---------------------------'
                print balance
                print '----------------------balance---------------------------'
                ...
                print pay_amt
                print '-----------------------------------------------------'
                msg = 'Packet(',src,',',dest,',',pay_amt,',',timestamp,')'
                msg = convertTuple(msg)
                msg = pubKey.encrypt(msg,32)
                msg = convertTuple(msg)
                print '-----------------------------------------------------'
                print (msg)
                print '-----------------------------------------------------'
                print key.decrypt(msg)
                sig = sk.sign(msg)
                msg = msg + '1111111' + sig 
                print '-----------------------sig------------------------------'
                print msg 
                print '-----------------------sig------------------------------'
                sock.sendto(msg, self.target)
            sys.stdout.flush()
